...
import { onCellClick, onWallClick, onWallOut, onWallOver, onBoardInit } from './engine_final.js';
...
export function display_board(board) {
    ...
    document.documentElement.style.setProperty('--number-of-row', board.getHeight());
        ...
        for (let i = 0; i < walls.length; i++) {
            for (let j = 0; j < walls[i].length; j++) {
                if (walls[i][j] != 0) {
                    ...
                    // set the small wall
                    let wall = document.getElementById('s-wall-' + walls[0][0] + '-' + walls[0][1]);
                    wall.classList.add('placed');
                }
            }
        }
